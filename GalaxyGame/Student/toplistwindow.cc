...
QList<QPair<QString, Student::Statistics::playerStat> > TopListWindow::sortTopTen(QMap<QString, Student::Statistics::playerStat> stats)
{
    ...
    {
        int first = it.value().completedQuest*3 + it.value().credits - it.value().lostShips + it.value().points*2 + it.value().savedShips*2;
        ...
        {
            int second = sortedStats.at(i).second.completedQuest*3 + sortedStats.at(i).second.credits - sortedStats.at(i).second.lostShips + sortedStats.at(i).second.points*2 + sortedStats.at(i).second.savedShips*2;
...
{
    out << playerStat.completedQuest << playerStat.credits << playerStat.lostShips << playerStat.points << playerStat.savedShips;
    return out;
}

QDataStream &operator>>(QDataStream &in, Student::Statistics::playerStat &playerStat)
{
    in >> playerStat.completedQuest >> playerStat.credits >> playerStat.lostShips >> playerStat.points >> playerStat.savedShips;
    return in;
}
